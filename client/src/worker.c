...
SMTP_connection_t** get_SMTP_connections(const tree_t* host_vs_peer) {
    ...
    int                 i;
    SMTP_connection_t** peers = (SMTP_connection_t**)malloc(
        sizeof(SMTP_connection_t*) * host_vs_peer->size);
    if (!peers) {
        sprintf(g_log_message,
                "Ошибка выделения памяти: get_SMTP_connections()");
        send_log();
        free(nodes);
        return NULL;
    }
...
peer_t** get_peers(SMTP_connection_t** conns, size_t size) {
    peer_t** peers =
        (peer_t**)malloc(sizeof(peer_t*) * (size + ADDITIONAL_PEERS_CNT));
    if (!peers) {
...
SMTP_connection_t* get_conn_for_sending(tree_t*         host_vs_conn_map,
    ...
    if (host_vs_peer) {
        ...
        if (peer_for_sending) {
            ...
            peer_for_sending =
                ((SMTP_connection_t*)tree_search(host_vs_conn_map, addr)
                     ->value);
        }
    }
    free_addr(addr);
...
int process_parent_messages(tree_t* host_vs_conn_map, queue_t* filenames,
    ...
    while (current_filename) {
        sprintf(g_log_message, "Найдено письмо для отправки: %s",
                current_filename->data);
        send_log();

        peer_for_sending =
            get_conn_for_sending(host_vs_conn_map, current_filename);
        if (peer_for_sending) {
...
int worker_main(const worker_config_t config) {
    ...
    tree_t* host_vs_conn_map = TREE_INIT(
        SMTP_connection_t, SMTP_connection_copy, SMTP_connection_clear);
    if (!host_vs_conn_map) {
        return MEMORY_ERROR;
    }
    ...
    WHILE_TRUE() {
        ...
        for (i = 0; i < peers_count + ADDITIONAL_PEERS_CNT; i++) {
            if (i == peers_count) {
                ...
                process_parent_messages(
                    host_vs_conn_map, peers[i]->messages_out, config.queue_dir);
                ...
                if (conns[i]->state == CLIENT_FSM_ST_CLOSED_CONNECTION) {
                    sprintf(g_log_message, "Соединение закрыто: %s (%s)",
                            conns[i]->addr, conns[i]->ip->data);
                    send_log();
